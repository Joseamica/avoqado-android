...
### Fixed
- **Reembolsar «sin tocar la propina» ya no ofrece un importe que el servidor rechaza.** El máximo de la hoja INCLUÍA la propina aunque la casilla «Incluir propina» estuviera apagada: con $200 + $20 el cajero dejaba $220, la app mandaba `tipRefundCents = 0` y el servidor contestaba «Sale portion of refund exceeds original sale amount» (Testarudo, 17-sep: cinco 400 seguidos y el reembolso salió 4.7 h después por otro camino). Ahora, con la propina apagada, el tope y «Disponible» son sólo la VENTA restante (`remainingRefundableSale`, que el servidor manda ya descontadas las devoluciones previas; con un servidor anterior, disponible − propina), y al apagar la casilla un importe que ya no cabe se recorta al tope — lo que cabe no se toca. Los artículos siempre son venta. Reglas puras en `RefundSubmitRules.kt`, con pruebas; espejo exacto en iOS.
